...
def paste_item(root):
    if len(root.Buffer) != 0:
        ...
        for i in root.Buffer:
            item = i[0]
            try:
                shutil.copy(DIR + "/" + item.Location + "/" + item.Name,
                            DIR + "/" + root.get_current_album(False).Location + "/" + item.Name[:-4] + "copy.jpg",
    ...
    if y < 33 and x < 33:
        if root.get_current_album(False) is not None:
            ...
            if root.get_current_album(False) is not None:
                ...
                print_albums(root)
            return
    if x > SCREEN_RESOLUTION[0] - 33 and y > SCREEN_RESOLUTION[1] - 33:
        if root.get_current_album(False) is not None:
            if root.get_current_album(False).try_get_next_list():
                print_albums(root.get_current_album(False))
            return
        else:
            if root.try_get_next_list():
                print_albums(root)
            return
    if x < 33 and y > SCREEN_RESOLUTION[1] - 33:
        if root.get_current_album(False) is not None:
            if root.get_current_album(False).try_get_previous_list():
                print_albums(root.get_current_album(False))
            return
        else:
            if root.try_get_previous_list():
                print_albums(root)
            return
    if root.get_current_album(False) is not None:
        item = get_album(root.get_current_album(False).Items[root.get_current_album(False).Pointer], pos)
